...
@section('content')
    ...
    {{-- Hero --}}
    <div class="bg-primary rounded-2xl p-8 mb-6 text-white">
        <div class="flex items-center gap-6">
            <div class="w-20 h-20 bg-white bg-opacity-20 rounded-2xl flex items-center justify-center flex-shrink-0">
            ...
            <div>
                <h2 class="text-2xl font-bold">Senegal EdTech</h2>
    ...
    <div class="grid grid-cols-3 gap-6 mb-6">
        ...
        <div class="bg-white rounded-2xl border border-border p-6 shadow-sm">
            <div class="w-10 h-10 bg-primary-bg rounded-xl flex items-center justify-center mb-4">
    ...
    <div class="bg-white rounded-2xl border border-border shadow-sm overflow-hidden mb-6">
        ...
        <div class="grid grid-cols-2 gap-0">
            ...
            <div class="flex items-start gap-4 p-6 {{ $i % 2 == 0 ? 'border-r' : '' }} border-b border-border">
    ...
    {{-- Stack technique + Version --}}
    <div class="grid grid-cols-2 gap-6">

        <div class="bg-white rounded-2xl border border-border shadow-sm p-6">
            <h3 class="font-bold text-text-dark text-base mb-4">Stack technique</h3>
            <div class="space-y-3">
                @foreach([
                    ['Laravel 12', 'Backend & Frontend (MVC)', 'bg-red-50 text-red-700'],
                    ['MySQL', 'Base de données relationnelle', 'bg-blue-50 text-blue-700'],
                    ['Tailwind CSS', 'Design & Interface utilisateur', 'bg-cyan-50 text-cyan-700'],
                    ['DomPDF', 'Génération des bulletins PDF', 'bg-green-50 text-green-700'],
                    ['Laravel Excel', 'Import des listes élèves', 'bg-amber-50 text-amber-700'],
                ] as $tech)
                <div class="flex items-center justify-between py-2 border-b border-border last:border-0">
                    <div class="flex items-center gap-3">
                        <span class="text-xs font-bold px-2.5 py-1 rounded-full {{ $tech[2] }}">{{ $tech[0] }}</span>
                        <span class="text-sm text-text-muted">{{ $tech[1] }}</span>
                    </div>
                </div>
                @endforeach
            </div>
        ...
        <div class="bg-white rounded-2xl border border-border shadow-sm p-6">
            ...
            <div class="space-y-3">
                @foreach([
                    ...
                    ['Plateforme', 'Web (Desktop)'],
                    ['Langue', 'Français'],
                    ['Niveaux supportés', 'CI, CP, CE1, CE2, CM1, CM2'],
                    ['Pays cible', 'Sénégal'],
                ] as $info)
                <div class="flex items-center justify-between py-2 border-b border-border last:border-0">
                    <span class="text-sm text-text-muted">{{ $info[0] }}</span>
                    <span class="text-sm font-semibold text-text-dark">{{ $info[1] }}</span>
                </div>
                @endforeach
            </div>
        </div>
